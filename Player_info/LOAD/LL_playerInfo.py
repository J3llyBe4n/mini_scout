...
# team_id 받아오기
getTeamApiIdNameSql = 'SELECT DISTINCT hidden_team_id.api_team_id, hidden_team_id.team_name FROM team_info  JOIN hidden_team_id ON team_info.team_name = hidden_team_id.team_name  WHERE team_info.league_id = 9 AND team_info.season_id = %d' %insertSeasonId
    ...
    for length in range(len(insertPlayerInfoList)):
        for m in range(len(mergeRaw)):
            if mergeRaw[m]['player']['name'] == insertPlayerInfoList[length][0]:
                ...
                print(insertPlayerInfoList[length][0])
                ...
                if tmpHeight is None or ord(tmpHeight[0]) == 10:
                    tmpHeight = 0                    
                else :
                    tmpHeight = tmpHeight.replace(' cm', '')
                print(tmpHeight)
                insertPlayerInfoList[length].append(int(copy.deepcopy(tmpHeight)))
                # Weight 값에 None 값이 존재해서 조건문 사용
                tmpWeight = mergeRaw[m]['player']['weight']
                if tmpWeight is None or ord(tmpWeight[0]) == 10:
                    tmpWeight = 0
                else :                
